"""
run_synthetic.py – run a synthetic experiment with the AMRIV estimator.
"""
### General imports
import os
import argparse
...
from utils import *
from data import *
from data import _gamma, true_pi
...
def one_run(seed: int,
    ...
    data_gen = make_synthetic_iv_dgp(f=my_f, d=d, seed=seed)
    exp = AMRIVExperiment(
            generator      = data_gen,
            n_rounds       = n_rounds,
            burn_in        = burn_in,
...
# Simulation Settings #
#######################
figs_dir = "./results/synthetic/figures"
results_dir = "./results/synthetic/logs"
d = 5
np.random.seed(1)
beta = np.random.uniform(-1, 1, size=d)
data_gen = make_synthetic_iv_dgp(f=my_f, d=5, seed=0)
...
oracle_factories = {
    ...
    "muY0": lambda **_: OracleMuY(z=0, f=my_f),
    "muY1": lambda **_: OracleMuY(z=1, f=my_f),
...
miss_factories = {
    ...
    #"muY1": lambda **_: LinearRegression(),
    "muA0": lambda **_: OracleMuA(z=0),
    "muA1": make_rf_factory(regression=False, n_estimators=100, max_depth=3, min_samples_leaf=30),
    "s0": make_rf_factory(regression=True, n_estimators=100, max_depth=5, min_samples_leaf=5),
    "s1": make_rf_factory(regression=True, n_estimators=100, max_depth=5, min_samples_leaf=5)
...
def main():
    ...
    if not plot_only:
        ...
        true_tau = true_ate(my_f, d, n=500000)
        true_Sigma = true_ate_var(my_f, d, n=50000)
        ##################
        # Run experiment #
        ##################
        #### Oracle-NA ####
    ...
    true_tau    = true_ate(my_f, d, n=500000)# assumed scalar in namespace
    true_Sigma  = true_ate_var(my_f, d, n=50000) # assumed asymptotic variance (scalar)
    # ---------- compute -------------------------------------------------
    ...
    plt.savefig(os.path.join(figs_dir, "Adaptivity_synthetic.pdf"), dpi=200, bbox_inches="tight")
    plt.savefig(os.path.join(figs_dir, "Adaptivity_synthetic.png"), dpi=200, bbox_inches="tight")
    plt.close()
    ...
    plt.grid(alpha=.3)
    plt.tight_layout(pad=0.3)
    plt.savefig(os.path.join(figs_dir, "Consistency_synthetic.pdf"), dpi=200, bbox_inches="tight")
    plt.savefig(os.path.join(figs_dir, "Consistency_synthetic.png"), dpi=200, bbox_inches="tight")
    plt.close()
    ...
    # ---------- plot: Coverage  -----------------------------------------
    names_to_plot = ["AMRIV", "AMRIV-NA", "AMRIV-MS",
                     "DM", "DM-NA", "DM-MS", "A2IPW"]
    
    plt.figure(figsize=(6, 4), dpi=100)
    handles = []
    labels = []
    for name in ["AMRIV", "AMRIV-NA", "AMRIV-MS", "DM", "DM-NA", "DM-MS", "A2IPW"]:
        root, spec = parse_label(name)
        colour = colour_of[root]
        marker, linestyle = style_of[spec]
        (line,) = plt.plot(
    ...
    plt.savefig(os.path.join(figs_dir, "Coverage_synthetic.pdf"), dpi=200,
                bbox_inches="tight", bbox_extra_artists=(legend,))
    plt.savefig(os.path.join(figs_dir, "Coverage_synthetic.png"), dpi=200,
                bbox_inches="tight", bbox_extra_artists=(legend,))
    plt.close()
    ...
    v0=4.0
    v1=1.0
    pi_star = np.sqrt(v0+(v1-v0)*gamma) / (np.sqrt(v0) + np.sqrt(v0+(v1-v0)*gamma))
    plt.plot(gamma, pi_star, label=r"$\pi^*(x)$", color="C0")
    plt.axhline(0.33333, color="black", linestyle="--")
    plt.text(0.0, 0.34, "Neyman Allocation")
    plt.ylabel(r"$\pi^*(x)$")
    plt.xlabel(r"$\delta^A(x)$")
    plt.title("Adaptive policy vs. compliance")
    plt.legend(loc="upper right", fontsize=12)
    plt.savefig(os.path.join(figs_dir, "Adaptive_policy_synthetic.pdf"), dpi=200,
                bbox_inches="tight")
    plt.savefig(os.path.join(figs_dir, "Adaptive_policy_synthetic.png"), dpi=200,
                bbox_inches="tight")
    plt.close()
